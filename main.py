...
TFIDF_MATRIX_PATH = os.path.join(BASE_DIR,'tfidf_matrics.pkl')
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not TMDB_API_KEY:
        raise HTTPException(
            status_code=500,
            detail="TMDB_API_KEY missing. Set it in .env as TMDB_API_KEY=xxx",
        )
    ...
    try:
        print(f"[DEBUG] Attempting TMDB request: {TMDB_BASE}{path}")
        print(f"[DEBUG] Params: {q}")
        async with httpx.AsyncClient(timeout=20, verify=False) as client:
            r = await client.get(f"{TMDB_BASE}{path}", params=q)
            print(f"[DEBUG] Response status: {r.status_code}")
    except httpx.ConnectError as e:
        import traceback
        print(f"[ERROR] ConnectError details: {e}")
        print(f"[ERROR] Traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=502,
            detail=f"TMDB ConnectError: {str(e)}",
        )
    except httpx.RequestError as e:
        import traceback
        print(f"[ERROR] RequestError details: {e}")
        print(f"[ERROR] Traceback: {traceback.format_exc()}")
        raise HTTPException(
            status_code=502,
